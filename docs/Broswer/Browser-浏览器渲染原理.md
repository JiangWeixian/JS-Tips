...
- [浏览器渲染原理](#浏览器渲染原理)
  ...
  - [浏览器渲染流程](#浏览器渲染流程)
    - [假设没有SCRIPT](#假设没有script)
    - [script的执行顺序](#script的执行顺序)
  - [QA](#qa)
    - [冷知识](#冷知识)
  - [渲染重新开始](#渲染重新开始)
  - [优化](#优化)
...
4. 初始页面渲染 = `cssdom htmldom`
5. 如果是`link html js`这样的结构，有没有`normal and defer or async`对**初始页面渲染**没影响，表现和`normal scipt`没啥区别。因为此时`cssdom htmldom`都已经解析完成了。之后运行代码可能会有闪烁。此时是二次渲染。
    * 之所以此时将`defer or async`的代码放在最后是为了兼容性考虑
    * 如果浏览器支持`defer or async`的话，放在开头位置会优化体验。因为优先解析到了`script`。
6. 如果是`js link html`和`link js html`两种结构，注意`link`的影响。只有这两种情况下`defer or async`才能够表现出优势。
7. `js`会等待前面的`css`
8. 在`DOMContentload`和`load`之间还有加载其他资源(包括图片&JS)的过程。
...
同时多个`defer`标签执行按顺序执行，`async`并不保证，谁先下载完之后再执行。

### 构建&渲染

#### 构建

由文件得到数据结构。指的是解析下载得到的`html`文件。

#### 渲染

就是可以看到呈现的画面 - 是`cssdom and htmldom`组合构成的行为。接下来分析一下`link html script`不同顺序对渲染结果的影响。

**如果是`link html script`这样的顺序。**

- **页面**中为`link html (no defer async)script`的构成。只会等待前面两个完成就会出现页面。见[(no defer async)script.html](https://github.com/JiangWeixian/JS-Tips/blob/master/docs/docs/Broswer/HTML/script.html)测试

因此如果`script`中存在修改样式代码，可能会出现页面重新绘制。
...
- 如果`link html (defer)script`只会等待前面两个完成就会出现页面([(调整网络为slow-3g)defer-script位置测试.html](https://github.com/JiangWeixian/JS-Tips/blob/master/docs/Broswer/HTML/defer-script%E9%A1%B5%E9%9D%A2%E4%BD%8D%E7%BD%AE%E6%B5%8B%E8%AF%95.html))。

- 如果是`async`由于加载完成时间不一致，可能还是会出现阻塞的情况([(调整网络为slow-3g)async-script位置测试.html](https://github.com/JiangWeixian/JS-Tips/blob/master/docs/Broswer/HTML/async-script%E9%A1%B5%E9%9D%A2%E4%BD%8D%E7%BD%AE%E6%B5%8B%E8%AF%95.html)都是比较大的文件，所以并没有出现阻塞情况)
...
* `DOMContentload` - `HTMLDOM`解析完毕就会触发(不需要等待样式表图片，样式表下载和解析属于同步过程，**但是需要等待所有的(normal and defer not async)JS文件执行完成**)。说明了`async`可能在`DOMContentload`之前或者之后。
* `load` - 在 `DOMContentload` 触发之后，加载完成所有资源(包括样式表图片，脚本文件)之后触发

`DOMContentload`和`script`之间关系。
...
综合`normal and defer script`测试可以得到结论：
...
> **💡 NOTE**  
由于DOMContentload与CSS无关，所以此时`DOMContentload`触发的时候并不一定会出现页面

## 浏览器渲染流程

![渲染流程图](/docs/Broswer/img/%E6%B5%8F%E8%A7%88%E5%99%A8%E6%B8%B2%E6%9F%93%E5%8E%9F%E7%90%86.png)
...
**同步&异步**，对于`script`脚本来说需要考虑同步异步，分别是加载以及执行的同步异步。这里只考虑加载的同步或者异步对于执行结果的影响。

对于同步加载的标签，假设`script`标签没有`defer or async`，那么在加载过程会阻塞`css or html`任何过程。所以`script`标签一般也是放在最后，保证`CSSDOM and HTMLDOM`先于`script`。

对于异步加载的标签，`async`加载时候不会阻塞，但是执行时候会，且和`normal`不会等待`htmldom`。所以可以放在头部，但是不应该有操作`css`的代码；如果放在尾部，表现和`normal`一致。

对于异步加载的标签，`defer`加载时候不会阻塞，但是执行时候会等待`htmldom`。所以可以放在任何位置。

不过此时需要考虑[Q&A - CSS和JS文件先后问题]()，JS只会等待前面`CSS`文件。

## QA

1. **如果JS下载完成，执行时间比较长，会影响dom吗？**
...
3. **`defer or async`JS和CSS书写顺序，同步异步关系**

4. **normal and defer or async下载的影响**

    我们知道`defer or async`下载过程对`dom`解析不受到影响。那么如果在`cssdom htmldom`解析完成之后，`js`仍旧没有下载完？那么页面还会出现吗？

    在[参考链接](https://juejin.im/post/59c60691518825396f4f71a1) 存在一个`script`在`cssdom htmldom`都完成了之后，还是处于下载过程中。此时页面出现，单等到该`script`下载完成，并执行，可能会让原先页面有闪硕。因为脚本文件中可能含有修改样式的代码。

    这一点和`defer normal async`的属性无关。

5. **defer or async下载是否会影响其他资源下载**
